...
clf
figSize = [12 6];
set(fA2, 'PaperUnits', 'inches');
set(fA2, 'Units','inches');
set(fA2, 'PaperSize', figSize);
set(fA2, 'PaperPositionMode', 'auto');
set(fA2, 'Position', [0 0 figSize(1) figSize(2)])

shockNames = {'FU', 'EBP', 'MU', 'Demand', 'Supply', 'Monetary', 'Financial'};
varNames = {'Uf', 'EBP', 'Um', 'IP', 'Prices', 'FFR', 'S&P500'};
...
for jj = 1:2 % Shocks: 1 = FU, 2 = EBP
    ...
    for ii = 1:n % Variables: n = 7
        ...
        percentiles_IP = prctile(squeeze(Draws_IRFs_narrative(ii,jj,1:hmaxtoplot+1,:)),bands,2);

        subplot(2,n,count)        
        plotConfidenceBandsBlue(0:hmaxtoplot,percentiles_IP,'r'); 
        hold on
        plot(0:hmaxtoplot,percentiles_IP(:,2),'r','LineWidth',2.5);
        hold on
