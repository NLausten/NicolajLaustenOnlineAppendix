...
Draws_HDs_narrative(:,:,draw) = squeeze(stochasticComponents(3,1:end,:)); % stochasticComponents:
end
...
varNames1 = {'Uncertainty','Credit Spread','Industrial Production','CPI','Policy Rate','Unemployment','Equity Prices'};
shockNames1 = {'Uncertainty','Credit Spread'};
...
for jj=1:2
    ...
    set(gca, 'FontSize', 12);
    set(gca,'Layer','top')
    set(gca,'YGrid','on')%,'YTick',[-25 -15 -5 0 5])
...
percentilesYY = sum(percentilesXX,3);
 ...
 hold on
   ...
   hold on
    ...
    set(gca, 'FontSize', 12);
    set(gca,'Layer','top')
    set(gca,'YGrid','on')%,'YTick',[-25 -20 -10 0 5])
    
count = count + 1;

tightfig;

%fname = strcat('results/HD_EU2Aunrate.eps');
%print('-depsc', f, fname);