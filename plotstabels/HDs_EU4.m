...
Draws_HDs_narrative(:,:,draw) = squeeze(stochasticComponents(5,1:end,:)); % stochasticComponents:
end
% 1.U, 2.EPU, 3.CS, 4. CC, 5.Y, 6.CPI, 7.PRATE, 8.UNRATE, 9.STOXX
...
varNames1 = {'Financial Uncertainty','EPU','Credit Spread','Consumer Confidence','Industrial Production'};
shockNames1 = {'Financial Uncertainty','EPU','Credit Spread','Consumer Confidence','Industrial Production'};
...
subplot(5,1,count)
percentilesXX = nan(length(Ytemp(:,1))-p+1,3,3);
for ij = 1:4
...
%fname = strcat('results/HD_EU4Aunrate.eps');
%print('-depsc', f, fname);